...
function AdminPanel({ employees, persistEmployees, punches, persistPunches, fetchLatestPunches, requests, persistRequests, leaves, persistLeaves, holidays, persistHolidays, fetchLatestHolidays, adminList, persistAdminList, fetchLatestAdminList, currentAdmin, storeCoords, persistStoreCoords, fetchLatestStoreCoords, overtimeCode, persistOvertimeCode, onExit }) {
  ...
  const pendingCount = requests.filter(r => r.status === "pendente" && (!restrictedStore || r.store === restrictedStore)).length;
  const todayKey = fmtDateKey(new Date());
  const onLeaveToday = leaves.filter(l => {
    if (l.startDate > todayKey || l.endDate < todayKey) return false;
    if (!restrictedStore) return true;
    const emp = employees.find(e => e.id === l.employeeId);
    return emp?.store === restrictedStore;
  }).length;
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16, animation: "slideUp 0.2s ease-out" }}>
      {currentAdmin && (
        <div style={{ color: COLORS.textDim, fontSize: 12 }}>
          Conectado como <b style={{ color: COLORS.text }}>{currentAdmin.name}</b>
          {restrictedStore && <> · administra <b style={{ color: COLORS.text }}>{STORES.find(s => s.id === restrictedStore)?.label}</b></>}
        </div>
      )}
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
...
// ---- Records tab ----
function RecordsTab({ employees, punches, persistPunches, leaves, fetchLatestPunches, restrictedStore }) {
  const [filterStore, setFilterStore] = useState(restrictedStore || "all");
  ...
  const visibleEmployees = useMemo(
    () => employees.filter(e => filterStore === "all" || e.store === filterStore).sort((a, b) => a.name.localeCompare(b.name)),
    [employees, filterStore]
  );
  ...
  const exportCSV = () => {
    ...
    const body = [...rows].sort((a, b) => new Date(a.at) - new Date(b.at)).map(p => {
...
function RequestsTab({ requests, persistRequests, punches, persistPunches, fetchLatestPunches, leaves, persistLeaves, currentAdmin, restrictedStore }) {
  ...
  const rows = useMemo(() => requests
    .filter(r => !restrictedStore || r.store === restrictedStore)
    .filter(r => filter === "all" || r.status === filter)
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)), [requests, filter, restrictedStore]);
...
function LeavesTab({ employees, leaves, persistLeaves, restrictedStore }) {
  ...
  const sorted = useMemo(() => {
    const scoped = restrictedStore ? leaves.filter(l => visibleEmployees.some(e => e.id === l.employeeId)) : leaves;
    return [...scoped].sort((a, b) => b.startDate.localeCompare(a.startDate));
  }, [leaves, restrictedStore, visibleEmployees]);
      ...
      {!showForm ? (
        ...
        <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 14, display: "flex", flexDirection: "column", gap: 8 }}>
          <select value={employeeId} onChange={e => setEmployeeId(e.target.value)} style={{ ...selectStyle, width: "100%" }}>
            ...
            {visibleEmployees.map(e => <option key={e.id} value={e.id}>{e.name} ({STORES.find(s => s.id === e.store)?.label})</option>)}
          </select>
          <select value={type} onChange={e => setType(e.target.value)} style={{ ...selectStyle, width: "100%" }}>
...
function ImportTab({ employees, punches, persistPunches, fetchLatestPunches, restrictedStore }) {
  ...
  const handleFile = async (e) => {
    ...
    const candidates = restrictedStore ? employees.filter(e => e.store === restrictedStore) : employees;
    const initialMapping = {};
    Object.keys(out.byName).forEach(name => {
      const target = norm(name);
      const exact = candidates.find(e => norm(e.name) === target);
      const contains = !exact && candidates.find(e => norm(e.name).includes(target) || target.includes(norm(e.name)));
...
function SettingsTab({ adminList, persistAdminList, fetchLatestAdminList, currentAdmin, storeCoords, persistStoreCoords, fetchLatestStoreCoords, restrictedStore, overtimeCode, persistOvertimeCode, employees, punches, requests, leaves, holidays, persistHolidays, fetchLatestHolidays }) {
  ...
  const updateManualCoord = (storeId, field, value) => setLocalCoords(prev => ({ ...prev, [storeId]: { ...(prev[storeId] || {}), [field]: value === "" ? undefined : -Math.abs(Number(value)) } }));
        ...
        {STORES.filter(s => !restrictedStore || s.id === restrictedStore).map(s => (
          <div key={s.id} style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 10 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
              <span style={{ fontSize: 12, color: COLORS.textDim }}>Ou digite manualmente (só os números, sem o sinal de menos):</span>
              <input type="number" step="0.00001" min="0" placeholder="Latitude" value={localCoords[s.id]?.lat != null ? Math.abs(localCoords[s.id].lat) : ""} onChange={e => updateManualCoord(s.id, "lat", e.target.value)} style={{ ...selectStyle, width: 130 }} />
              <input type="number" step="0.00001" min="0" placeholder="Longitude" value={localCoords[s.id]?.lng != null ? Math.abs(localCoords[s.id].lng) : ""} onChange={e => updateManualCoord(s.id, "lng", e.target.value)} style={{ ...selectStyle, width: 130 }} />
            </div>
